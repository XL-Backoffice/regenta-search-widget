...
export default function Search({ page }) {
  ...
  const [selected, setSelected] = useState(child_age[3]);
  ...
  //       return title.includes(searchQuery) || state.includes(searchQuery);
  //     });
  useEffect(() => {
    getHotels();
    // getCities();
  ...
  useEffect(() => {
    if (
      userInfo?.type === "bookingAgent" ||
      (searchFields?.corporateCode && searchFields?.corporateCode !== "")
    ) {
      setNumberOfRooms(10);
    }
  }, [userInfo, searchFields]);
  ...
  // const getCities = () => {
  //   configuration
  //     .getAPI({ url: "api/common/cities", params: {} })
  //     .then((data) => {
  //       if (data?.payload) {
  //         setCities(data?.payload);
  //         const popular = data?.payload?.filter((single) => single?.isPopular);
  //         setPopularCity(popular);
  //       } else if (data?.error) {
  //         return toast.error(data?.error?.message);
  //       } else {
  //         return toast.error("Something went wrong");
  ...
  return (
    <>
      ...
      >
        {/* <div className='absolute z-10 top-2 left-2 stikeybar-logo'><Link to="/"><img loading="lazy" className="w-20 xl:w-36" src="/images/logo.png" alt="" /></Link></div> */}
                              ...
                              >
                                <li className="p-2">
                                  {!isAllowLocation && (
                                    <div
                                      onClick={() => getLocation()}
                                      className="mb-2 mt-4 px-3 text-sm font-semibold cursor-pointer flex primary-color"
                                    >
                                      <svg
                                      ...
                                      >
                                        <path
                                          fill="currentColor"
                                          d="M10.368 19.102c.349 1.049 1.011 1.086 1.478.086l5.309-11.375c.467-1.002.034-1.434-.967-.967L4.812 12.154c-1.001.467-.963 1.129.085 1.479L9 15z"
                                        />
                                      </svg>{" "}
                                    ...
                                    <>
                                      ...
                                      >
                                        ...
                                        <div className="text-center py-4">
                                          <Link
                                            className="flex justify-center primary-color"
                                            to="/destinations"
                                          >
                                            View All{" "}
                                            <ChevronRightIcon
                                        ...
                                        <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                                          ...
                                          >
                                            ...
                                            <div className="text-right border-t pt-3 border-gray-200 mt-3">
                                              {/* <PopoverButton className="outline-0">
                                        <FillButton text="Add" className='fill_btn pb-2' />
                                      </PopoverButton> */}
